...
public final class Signal<Value, Error: Swift.Error> {
	...
	///
	/// - parameters:
	///   - strategy: The strategy of the signal to serialize its received events.
	///   - generator: A closure that accepts an implicitly created observer
	///                that will act as an event emitter for the signal.
	public init(serialization strategy: SerializationStrategy = .serialize, _ generator: (Observer) -> Disposable?) {
	...
	public static func pipe(serialization strategy: SerializationStrategy = .serialize, disposable: Disposable? = nil) -> (Signal, Observer) {
		var observer: Observer!
		let signal = self.init(serialization: strategy) { innerObserver in
